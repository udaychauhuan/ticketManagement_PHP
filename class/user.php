  ...
  public function create_user($data)
  {
    ...
    if (!$result1) {
      ...
      $query = "INSERT INTO `user_table` ( `name`, `email`, `phone`,  `role_id`, `password`)
            SELECT * FROM (SELECT '$name' AS name, '$email' AS email,'$phone' AS phone,'$role' AS role_id , '$password' AS password) AS tmp WHERE NOT EXISTS (SELECT email FROM user_table WHERE email = '$email') LIMIT 1";
      $result = $conn->save($query);
      if ($result) {
        return TRUE;
      } else { //  echo "email is already exists .";
        return false;
      }
    } else {
  ...
  public function update_user($data, $id)
  {
    ...
    if (is_array($result1)) {
      foreach ($result1 as $key) {
        # code...
        if ($email == $key['email']) {
          // means  id  is present in the database
          $check_mail++;
        } else {
